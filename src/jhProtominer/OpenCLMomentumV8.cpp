...
OpenCLMomentumV8::OpenCLMomentumV8(int _HASH_BITS, int _device_num) {
	...
	// compiles
	fprintf(stdout, "Starting OpenCLMomentum V8\n");
	...
	program_filenames.push_back("opencl/OpenCLMomentumV8.cl");
...
OpenCLMomentumV8::~OpenCLMomentumV8() {
	// destroy
	delete hashes;
	delete hash_table;
	delete (collisions);
...
int OpenCLMomentumV8::getCollisionCeiling() {
	return (1<<8);
}
